...
class Retriever:
    ...
    """
    TO DO:
    - see how much is repeated between Observatory.test_psf and the psf model 
    generation here?
    """
    ...
    def __init__(self, target,
        ...
        self.psf_model_fname = os.path.join(self.results_dir, 
                                            'hlc_centered_psf_OS11_20240515.fits')
        
        # Generate centered PSF model if it does not exist
            
        if os.path.exists(self.psf_model_fname):
            print(f'PSF model already exists at: {self.psf_model_fname}')  

        else:
            # Load oversampled PSF model
            hlc_psf_path =  pkg_resources.resource_filename('exoscene', 'data/cgi_hlc_psf')
            offset_psfs_fname = os.path.join(hlc_psf_path, 'hlc_os11_psfs_oversampled.fits')
            offset_psfs_offsets_fname = os.path.join(hlc_psf_path, 'hlc_os11_psfs_radial_offsets.fits')

            #cx_os6 = 100 # True center of the high-res OS6 array,
            #             # derived from checking the peak-offset correspondence in the off-axis PSF model. 
                
            pixscale_as = (fits.getheader(offset_psfs_fname))['PIXAS']
            pixscale_LoD = (fits.getheader(offset_psfs_fname))['PIXLAMD']

            offset_psfs_os11 = fits.getdata(offset_psfs_fname)
            offsets_LoD = fits.getdata(offset_psfs_offsets_fname)
            offsets_as = offsets_LoD * pixscale_as / pixscale_LoD

            # Apply half-pixel shift to OS11 arrays to place center at (99.5, 99.5)
            ### Do we need to do this? How to check? ###
            #offset_psfs = scipy.ndimage.interpolation.shift(offset_psfs_os11, (0.,0., -0.5, -0.5),
            #                                                order = 1, prefilter=False,
            #                                                mode = 'constant', cval=0)

        
            
            # Find the least distorted PSF model
            r_as = np.mean([self.configs['iwa_mas'],self.configs['owa_mas']]) / 1000 # radius halfway b/n IWA & OWA in arcsec
            r_p = int(np.round(r_as / pixscale_as)) # radius halfway in pixels
            oi = np.argmin(np.abs(r_as - offsets_as)) # index of closest radial offset in Krist model array
            dr_as = r_as - offsets_as[oi] # radial shift needed in arcseconds
            dr_p = dr_as / pixscale_as # radial shift needed in pixels

            shift_psf = scipy.ndimage.interpolation.shift(offset_psfs_os11[0,oi], (0, dr_p),
                                                          order = 1, prefilter=False)
